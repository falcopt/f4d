...
namespace cobra {
    ...
    public:
        TailsExchange(const Instance &instance_, MoveGenerators &moves_, float tolerance_) : AbstractOperator(instance_, moves_, tolerance_) { }
    ...
    protected:
        ...
        inline float compute_cost(const Solution &solution, const MoveGenerator &move) override {
            ...
            const auto delta = -solution.get_next_cost(i) + this->instance.get_cost(i, j) - solution.get_prev_cost(j) +
