...
namespace cobra {
    ...
    public:
        RevTwoOneExchange(const Instance &instance_, MoveGenerators &moves_, float tolerance_) : AbstractOperator(instance_, moves_, tolerance_) { }
    ...
    protected:
        ...
        inline float compute_cost(const Solution &solution, const MoveGenerator &move) override {
            ...
            const auto jPrevRem = -solution.get_next_cost(j) - solution.get_next_cost(jNext);
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex) {
            ...
            c.seqrem = -solution.get_prev_cost(c.prev, c.prevprev) - c_v_next;
            c.prevrem = -c_v_next - solution.get_next_cost(c.next, c.nextnext);
