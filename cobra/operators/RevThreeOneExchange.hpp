...
namespace cobra {
    ...
    public:
        RevThreeOneExchange(const Instance &instance_, MoveGenerators &moves_, float tolerance_) : AbstractOperator(instance_, moves_, tolerance_) { }
    ...
    protected:
        ...
        inline float compute_cost(const Solution &solution, const MoveGenerator &move) override {
            ...
            const auto jSequenceRem = -solution.get_next_cost(j) - solution.get_next_cost(jNext);
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex) {
            ...
            // c.v = i in (i, j)
            c.sequrem = -solution.get_prev_cost(c.prevprev, c.prevprevprev) - c_v_next;
            // c.v = j in (i, j)
            c.nextrem = -c_v_next - solution.get_next_cost(c.next,c.nextnext);
        ...
        inline Cache1 prepare_cache1(const Solution &solution, int vertex) {
            ...
            c.prevprev = solution.get_prev_vertex(route, prev);
            c.prevprevprev = solution.get_prev_vertex(route, c.prevprev);
            c.sequrem = -solution.get_prev_cost(c.prevprev, c.prevprevprev) - solution.get_next_cost(c.v);
            return c;
        }
        ...
        inline Cache1 prepare_cache1(const Solution &solution, int vertex, int backup) {
            ...
            c.prevprev = solution.get_prev_vertex(route, prev);
            c.prevprevprev = solution.get_prev_vertex(route, c.prevprev);
            c.sequrem = -solution.get_prev_cost(c.prevprev, c.prevprevprev) - solution.get_next_cost(c.v, c.next);
            return c;
        }
        ...
        inline Cache2 prepare_cache2(const Solution &solution, int vertex) {
            ...
            const auto route = solution.get_route_index(c.v);
            c.nextnext = solution.get_next_vertex(route, c.next);
            c.nextrem = -solution.get_next_cost(c.v) - solution.get_next_cost(c.next, c.nextnext);
            return c;
        }
        ...
        inline Cache2 prepare_cache2(const Solution &solution, int vertex, int backup) {
            ...
            c.next = solution.get_next_vertex(route, c.v);
            c.nextnext = solution.get_next_vertex(route, c.next);
            c.nextrem = -solution.get_next_cost(c.v, c.next) - solution.get_next_cost(c.next, c.nextnext);
            return c;
        }
