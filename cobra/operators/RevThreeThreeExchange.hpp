#ifndef _F4D_REVTHREETHREEEXCHANGE_HPP_
#define _F4D_REVTHREETHREEEXCHANGE_HPP_
...
namespace cobra {
    ...
    public:
        RevThreeThreeExchange(const Instance &instance_, MoveGenerators &moves_, float tolerance_) : AbstractOperator(instance_, moves_, tolerance_) { }
    ...
    protected:
        ...
        inline float compute_cost(const Solution &solution, const MoveGenerator &move) override {
            ...
            const auto jSequenceRem = -solution.get_next_cost(j) - this->instance.get_cost(jNextNextNext, jNextNextNextNext);
        ...
        bool is_feasible(const Solution &solution, const MoveGenerator &move) override {
            ...
            return (iRoute != jRoute && iPrev != this->instance.get_depot() && iPrevPrev != this->instance.get_depot() && jNext != this->instance.get_depot() &&
                    jNextNext != this->instance.get_depot() && jNextNextNext != this->instance.get_depot() &&
                    ...
                    solution.get_route_load(iRoute) + this->instance.get_demand(jNext) + this->instance.get_demand(jNextNext) +
                            ...
                            this->instance.get_demand(iPrevPrev) <=
                        this->instance.get_vehicle_capacity()) ||
                   (iRoute == jRoute && j != iPrev && j != iPrevPrev && jNext != iPrevPrev && jNextNext != iPrevPrev && jNextNextNext != iPrevPrev &&
        ...
        inline Cache12 prepare_cache12(const Solution &solution, int vertex) {
            ...
            // c.v = j in (i, j)
            c.seq2rem = -c_v_next - this->instance.get_cost(c.nextnextnext, c.nextnextnextnext);
        ...
        inline Cache2 prepare_cache2(const Solution &solution, int vertex) {
            ...
            c.nextnextnext = solution.get_next_vertex(route, nextnext);
            c.nextnextnextnext = solution.get_next_vertex(route, c.nextnextnext);
            c.seq2rem = -solution.get_next_cost(c.v) - this->instance.get_cost(c.nextnextnext, c.nextnextnextnext);
            return c;
        }
        ...
        inline Cache2 prepare_cache2(const Solution &solution, int vertex, int backup) {
            ...
            c.nextnextnext = solution.get_next_vertex(route, nextnext);
            c.nextnextnextnext = solution.get_next_vertex(route, c.nextnextnext);
            c.seq2rem = -solution.get_next_cost(c.v, c.next) - this->instance.get_cost(c.nextnextnext, c.nextnextnextnext);
            return c;
        }
