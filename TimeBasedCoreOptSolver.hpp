...
#ifdef VERBOSE
            partial_time_end = std::chrono::high_resolution_clock::now();
            const auto print_elapsed_time = std::chrono::duration_cast<std::chrono::seconds>(partial_time_end - partial_time_begin).count();
            if (print_elapsed_time > 1) {
...
private:
    ...
    void print_progress(cobra::PrettyPrinter& printer, int iter, const int iterations,
        ...
        const auto progress = 100.0f * elapsed_time / iterations;
        ...
        printer.print(progress, iter + 1, bks.has_value() ? 100.0f * (best_solution.get_cost() - *bks) / *bks : best_solution.get_cost(),
                      best_solution.get_routes_num(), iter_per_second, estimated_rem_time, gamma_mean, omega_mean, sa.get_temperature(elapsed_time));
    }
};
